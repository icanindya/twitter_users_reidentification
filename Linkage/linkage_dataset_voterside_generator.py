...
fieldnames = ['voter_serial', 'fname', 'mname', 'lname', 'sex', 'race',
              'yob', 'party', 'city']
    ...
    for filenname in os.listdir(FL_EXT_PATH):
        with open(os.path.join(FL_EXT_PATH, filenname), 'r') as rf:
            for line in rf:
                ...
                lname = tokens[3]
                sex = tokens[4]
                race = helper.get_race_label(int(tokens[6]))
                yob = tokens[5][-4:] if tokens[5] else ''
                party = tokens[12]
                city = tokens[9]
                zip = tokens[10]
                county = tokens[11]
                ...
                writer.writerow([voter_serial, fname, mname, lname, sex, race,
                                 yob, party, city])
